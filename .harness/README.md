...
`merged-deploy.yaml` is the normal deployment pipeline. It verifies a protected normal merge, then uses installed trusted code to validate, update and prove the repeat is a no-op. The user has approved baseline and incremental-index acceptance. `merged-trigger.yaml` is initially disabled for replication; enable only after baseline acceptance. Deployment results are recorded in the Git automation guide.
